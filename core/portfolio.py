...
def calculate_kelly_size(
    ...
    """
    Fat-tail Kelly position sizing.

    Returns:
        (shares, risk_inr, kelly_f, kurt_correction)

    Args:
        entry / stop:     Price levels — rps = |entry - stop|
        prob_win:         Win probability from Platt model
        rr:               Reward:Risk ratio to T1
        daily_df:         Daily OHLCV with Close — used for kurtosis
        config:           SystemConfig instance
        regime:           Market regime string for capital scaling
        capital_fraction: Fraction of risk capital to deploy (0–1).
                          Pass from CapitalScaler.capital_fraction() if
                          tracking live NAV; defaults to 1.0 for backtests.
    """
    rps = abs(entry - stop)
    if rps <= 0:
    ...
    # Fat-tail correction: k=3 (normal) → correction=1.0, higher kurtosis → lower fraction
    ...
    risk_inr = min(risk_inr, config.RISK_PER_TRADE_INR * config.KELLY_MAX_MULT * capital_fraction)
    risk_inr = max(risk_inr, config.RISK_PER_TRADE_INR * 0.25 * capital_fraction)

    shares = max(config.KELLY_MIN_SHARES, int(risk_inr / rps)) if capital_fraction > 0 else 0
...
def optimize_portfolio(
    ...
    Select up to config.PORTFOLIO_SIZE tickers from candidates, subject to:
      ...
      2. Correlation — skip any ticker with |corr| > config.MAX_CORR
                       vs an already-selected ticker (MAX_CORR gate was
                       defined in CONFIG but never applied before v14)

    Args:
        candidates:   List of TickerResult — sorted by sharpe_rank desc
        config:       SystemConfig
        corr_matrix:  Pairwise correlation matrix of daily returns.
                      Index/columns should be "{ticker}.NS" format.
                      Pass pd.DataFrame() or None to skip correlation filter.
    ...
    for c in sorted_candidates:
        ...
        # Correlation filter  ← wired for the first time in v14
